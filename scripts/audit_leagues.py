...
ingest one name at a time.
    ...
    python scripts/audit_leagues.py --seasons 3     # quick smoke test only

Audit the SAME number of seasons the ingest will pull. A 3-season audit
only sees clubs that played in those seasons -- anything relegated earlier
(Schalke, Hertha, Bordeaux, Sampdoria, Cadiz) goes unchecked, and then the
full ingest fails validation on a name nobody looked at.
...
from plfc.fdcouk import FootballDataCoUkError, read_league_season  # noqa: E402
from plfc.ingest import season_range  # noqa: E402
from plfc.leagues import DEFAULT_LEAGUES, config  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--seasons", type=int, default=10,
                    help="Must match what the ingest will pull, or clubs "
                         "relegated before the window go unchecked.")
    ...
    for lg in leagues:
        ...
        for season in seasons:
            try:
                frames.append(read_league_season(cfg.code, season))
        ...
        names = sorted(set(raw["home_team_raw"]) | set(raw["away_team_raw"]))
    ...
    print("\n" + "=" * 66)
    if args.seasons < 10:
        print(f"NOTE: audited only {args.seasons} season(s). Clubs that left "
              f"these leagues earlier were never checked, and a full ingest "
              f"will hit them. Re-run with --seasons 10 before ingesting.\n")
